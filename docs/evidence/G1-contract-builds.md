...
- G1 code tasks complete; gate exit still requires the independent read-only review mandated by the roadmap, then lead rerun. Android SDK/JDK live under `~/Android/` and are outside repository state — CI provisions its own via setup-android/temurin 21.
